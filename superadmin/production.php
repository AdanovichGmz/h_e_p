...
require('../saves/conexion.php');
    ...
    $query1 = "SELECT o.*,p.proceso,p.id_proceso,(SELECT orden_display FROM orden_estatus WHERE id_orden=o.idorden AND id_proceso=p.id_proceso) AS orden_display,(SELECT status FROM orden_estatus WHERE id_orden=o.idorden AND id_proceso=p.id_proceso) AS status FROM ordenes o INNER JOIN procesos p ON p.id_orden=o.idorden WHERE nombre_proceso='$maquina' HAVING status='actual' ";
    
    $actual = $mysqli->query($query1);
     
    $query2 = "SELECT o.*,p.proceso,p.id_proceso,(SELECT orden_display FROM orden_estatus WHERE id_orden=o.idorden AND id_proceso=p.id_proceso) AS orden_display,(SELECT status FROM orden_estatus WHERE id_orden=o.idorden AND id_proceso=p.id_proceso) AS status FROM ordenes o INNER JOIN procesos p ON p.id_orden=o.idorden WHERE nombre_proceso='$maquina' HAVING status='siguiente' ";
    
    $siguiente = $mysqli->query($query2);

    $query3 = "SELECT o.*,p.proceso,p.id_proceso,(SELECT orden_display FROM orden_estatus WHERE id_orden=o.idorden AND id_proceso=p.id_proceso) AS orden_display,(SELECT status FROM orden_estatus WHERE id_orden=o.idorden AND id_proceso=p.id_proceso) AS status FROM ordenes o INNER JOIN procesos p ON p.id_orden=o.idorden WHERE nombre_proceso='$maquina' HAVING status='preparacion' ";
    
    $preparacion = $mysqli->query($query3);

     $row = mysqli_fetch_object($actual);
      $act = (!empty($row->numodt))? $row->numodt : '--';
      $nom_act=(!empty($row->producto))? $row->producto : '';
      $element   = $idmaquina;
    
      
      $row2 = mysqli_fetch_object($siguiente);
      $sig = (!empty($row2->numodt))? $row2->numodt : '--';
      $nom_sig=(!empty($row2->producto))? $row2->producto : '';
            
          
      $row3 = mysqli_fetch_object($preparacion);
      $prep = (!empty($row3->numodt))? $row3->numodt : '--';
      $nom_prep=(!empty($row3->producto))? $row3->producto : '';
    ...
    $etequery3 = "SELECT COALESCE(((SELECT SUM( entregados ) FROM tiraje WHERE id_maquina=$idmaquina AND fechadeldia_tiraje = '$today')-(SELECT SUM( defectos ) FROM tiraje WHERE id_maquina=$idmaquina AND fechadeldia_tiraje = '$today'))*100/ (SELECT SUM( cantidad ) FROM tiraje WHERE id_maquina=$idmaquina AND fechadeldia_tiraje = '$today')) as calidad_primera";

    $etequery4 = "SELECT COALESCE(((SELECT SUM(entregados) FROM tiraje WHERE id_maquina=$idmaquina AND fechadeldia_tiraje = '$today')+(SELECT SUM(merma) FROM tiraje WHERE id_maquina=$idmaquina AND fechadeldia_ajuste = '$today'))) as desempenio";
     
    
    
    
    //$standar_query="SELECT * FROM estandares WHERE id_maquina=$machineID AND id_elemento=$element ";
  ...
  /****************** Calcular ETE ******************/
    ...
    <div class="ete-stat">
      ...
      <tr class="trh">
        ...
        <tr class="trb">
          <td>'.$act." ".$nom_act.'</td>
          <td class="middletd">'.$sig." ".$nom_sig.'</td>
          <td>'.$prep." ".$nom_prep.'</td>
        </tr>
        </tbody>
      </table>
